...
def gen_agl_map(inputs, height, width,feature_dims):
    with tf.compat.v1.name_scope("gen_agl_map"):
...
def apply_lcm(batch_img, light_weight):
    with tf.compat.v1.name_scope('apply_lcm'):
...
def trans_module(inputs, structures, phase_train, name="trans_module"):
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        cnn_4 = tf.compat.v1.layers.conv2d(inputs=cnn_blk_3, filters=structures['depth'][4], kernel_size=structures['filter_size'][4], padding="same", activation=None, use_bias=False, name="cnn_4")
        return cnn_4
...
def lcm_module(inputs, structures, phase_train, name="lcm_module"):
    with tf.compat.v1.variable_scope(name) as scope:
        cnn_blk_0 = tf_utils.cnn_blk(inputs, structures['depth'][0], structures['filter_size'][0], phase_train, name = 'cnn_blk_0')        
        cnn_blk_1 = tf_utils.cnn_blk(cnn_blk_0, structures['depth'][1], structures['filter_size'][1], phase_train, name = 'cnn_blk_1')
        cnn_2 = tf.compat.v1.layers.conv2d(inputs=cnn_blk_1, filters=structures['depth'][2], kernel_size=structures['filter_size'][2], padding="same", activation=None, use_bias=False, name='cnn_2')
...
def inference(input_img, input_fp, input_agl, phase_train, conf):
    ...
    with tf.compat.v1.variable_scope('warping_model'):        
        ...
        with tf.compat.v1.variable_scope('warping_module'):
            '''coarse module'''
            resized_igt_inputs = tf.compat.v1.layers.average_pooling2d(inputs=igt_inputs, pool_size=[2,2], strides=2, padding='same')
            ...
            coarse_out = tf.compat.v1.layers.average_pooling2d(inputs=coarse_resize, pool_size=[2, 2], strides=1, padding='same')
            '''fine module'''
...
def dist_loss(y_pred, y_, method="MAE"):
    with tf.compat.v1.variable_scope('img_dist_loss'):
        loss = 0
        if(method == "L2"):
            loss = tf.sqrt(tf.reduce_sum(tf.square(y_pred - y_), axis=3, keepdims = True))
        elif (method == "MAE"):
...
def loss(img_pred, img_, eye_mask, input_img, flow, lcm_wgt):
    with tf.compat.v1.variable_scope('losses'):
        ...
        tf.compat.v1.add_to_collection('losses', losses)
        return tf.add_n(tf.compat.v1.get_collection('losses'), name='total_loss'), loss_img