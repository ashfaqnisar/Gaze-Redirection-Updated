...
class gaze_redirection_system:
    ...
    def flx_gaze(self, frame, gray, detections, shared_v, lock, pixel_cut=[3,4], size_I = [48,64]):
        ...
        for k,bx in enumerate(detections):
            ...
            # frame[R_le_LT[0] + pixel_cut[0]:R_le_LT[0] + size_le_ori[0]
            #     - pixel_cut[0], R_le_LT[1] + pixel_cut[1]:R_le_LT[1]
            #     + size_le_ori[1] - pixel_cut[1]] = LE_infer[pixel_cut[0]:,
            #     pixel_cut[1]:] * 255

            # frame[R_re_LT[0] + pixel_cut[0]:R_re_LT[0] + size_re_ori[0]
            #     - pixel_cut[0], R_re_LT[1] + pixel_cut[1]:R_re_LT[1]
            #     + size_re_ori[1] - pixel_cut[1]] = RE_infer[pixel_cut[0]:,
            #     pixel_cut[1]:] * 255
